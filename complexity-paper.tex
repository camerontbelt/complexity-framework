...
The composite is validated across five substrates:
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Spatial Prisoner's Dilemma (temptation sweep $T\in[1.0,2.0]$):}
    The composite peaks at $T\!=\!1.80$ with mean $C\!=\!1.06$
    ($\approx$90\% of random initial conditions reach the complex basin),
    matching the critical regime independently identified by
    Nowak and May~\cite{nowak1992} from evolutionary game theory.
    Three control games (Stag Hunt, Minority game, ordered cooperation)
    correctly return $C\!\approx\!0$.
    This experiment constitutes the first direct validation of
    candidate property P6 (co-evolutionary dynamics).
\end{itemize}
...
Version~5 introduced temporal opacity, fractal dimension,
the additive opacity architecture, the triple-tcomp Gaussian, and
extended validation to synthetic EEG data.
The 2D Life-like result improved from F1\,=\,0.800 to F1\,=\,1.000.
Version~9 (this paper) adds the spatial Prisoner's Dilemma experiment
(Experiment~5), constituting the first direct validation of P6
(co-evolutionary dynamics), and demonstrates cross-domain convergence
with the Nowak-May critical temptation $T\!\approx\!1.8$.
The Langton $\lambda$ analysis confirms $C$ is not rediscovering
$\lambda$'s structural property.
...
Maximum complexity is determined by the richness of external
relationships; complexity is fundamentally ecological rather than
individual.

\textit{Experimental validation (Experiment~5):}
The spatial Prisoner's Dilemma constitutes a direct operationalisation
of P6.
Cooperators and defectors occupy the same spatial environment; each
population continuously reshapes the fitness landscape of the other.
The composite $C$ peaks at $T\!=\!1.80$, precisely when this mutual
evolutionary pressure generates propagating cooperation-defection fronts.
When the pressure is absent --- cooperation trivially dominates
($T\!<\!1.78$), defection trivially dominates ($T\!=\!2.0$), or
opposition is symmetric (minority game) --- $C$ correctly returns zero.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{opacity_variants_analysis.png}
\caption{Bidirectional spatial opacity analysis across all 256 ECA rules.
\textit{Top left:} Scatter of upward ($H(\text{global}\mid\text{local})$)
vs downward ($H(\text{local}\mid\text{global})$) opacity; C4 cluster
annotated (intermediate upward, high downward).
\textit{Bottom:} Violin distributions by class for each direction, and
the composite product in the full framework.
C4 and C3 are nearly identical on downward opacity alone; upward
opacity is what separates them.}
\label{fig:opacity_variants}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{excess_entropy_analysis.png}
\caption{Temporal opacity (excess entropy) analysis.
\textit{Top left:} MI at lag~1 vs decay for 2D Life-like rules;
C4 rules cluster in the zone of moderate MI with positive decay.
\textit{Top centre:} $w_{\text{excess}}$ per rule; C4 rules dominate.
\textit{Top right:} Dimensional scaling --- $w_{\text{excess}}$ is
near-zero for all 1D ECA classes but activates strongly in 2D.
\textit{Bottom:} The $w_{\text{excess}}$ weight function and its
three parameter-free gate boundaries.}
\label{fig:excess_entropy}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{fractal_dim_analysis.png}
\caption{Fractal dimension metric ($w_\text{dim}$) analysis.
\textit{Top left:} Box-counting dimension by Wolfram class; C4 rules
cluster at intermediate values ($d_f\approx1.3$--$1.4$).
\textit{Top centre:} The Gaussian weight function peaking at excess~$\approx0.35$.
\textit{Top right:} F1 score with vs without $w_\text{dim}$; adding the
metric raises 2D Life F1 from 0.400 to 0.800 while correctly
penalising space-filling Class~3/2 rules.
\textit{Bottom:} Composite $C$ before vs after $w_\text{dim}$.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{multiplicative_vs_additive.png}
\caption{Multiplicative vs additive composite architectures.
\textit{Top:} AUC and F1 comparison across six combination variants
for 1D ECA and N-body substrates.
The multiplicative composite maintains the highest N-body F1 (0.959)
while matching alternatives on 1D ECA (all reach F1\,=\,1.000).
\textit{Bottom left:} Effect of a near-zero weight on each combination:
only the multiplicative variant collapses the score to zero, correctly
encoding the non-eliminability claim.
\textit{Bottom right:} Verdict table.}
\label{fig:mult_vs_add}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{robustness_tests.png}
\caption{Robustness tests for the 1D ECA composite.
\textit{Test 1:} IC density sweep --- F1\,=\,1.000 is maintained
across densities 0.1--0.9 (single failure at $d=0.3$ noted and
explained by Rule~137 displacement).
\textit{Test 2:} Grid size sweep --- C4/C3 separation remains 27--40$\times$
across $W=100$--$300$.
\textit{Test 3:} Shuffle test --- composite $C$ collapses when spatial
or temporal structure is destroyed, confirming the metric detects
genuine structure rather than statistics.
\textit{Tests 4a/4b:} Metric correlation matrices before and after
Gaussian weighting.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{v6_full_results.png}
\caption{Framework v6: metric weights across substrates.
\textit{Top left:} Mean metric weights per class in 1D ECA; C4
separation preserved (F1\,=\,1.000, $d$\,=\,29.6).
\textit{Top centre:} Spatial vs temporal opacity scatter (all 256 ECA
rules); C4 cluster annotated at high spatial, near-zero temporal ---
confirming dimensional scaling.
\textit{Top right:} 2D Life metric weight heatmap; $w_{\text{OP},t}$
perfectly separates C4 from C3/frozen-C2.
\textit{Bottom:} $w_{\text{OP},t}$ by class and substrate showing the
key finding: activates strongly in 2D, near-zero in 1D.}
\label{fig:v6_results}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{v7_analysis.png}
\caption{Framework v7: additive opacity and triple tcomp.
\textit{Top left:} Composite architecture evolution v5$\to$v7 with
rationale for additive vs multiplicative opacity.
\textit{Top centre:} 1D ECA v5 vs v7 scores (C4 preserved, diagonal
confirms no regression).
\textit{Top right:} 2D Life-like rules ranked by v7 composite; all
four C4 rules in top positions.
\textit{Bottom:} F1 progression across architectural changes and
the Seeds edge case analysis.}
\label{fig:v7_analysis}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{complexity_statistical_analysis.png}
\caption{N-body statistical analysis.
\textit{Top left:} Complexity score by physical state; $p\!=\!1.33\!\times\!10^{-17}$, $d\!=\!3.89$.
\textit{Top centre:} AUC per sub-metric and composite (Active vs Non-Active); composite AUC\,=\,0.991.
\textit{Centre left:} Complexity heatmap across $\alpha$--$\alpha_s$ parameter space; our universe marked.
\textit{Centre:} F1 comparison --- composite (0.949) vs analytical island baseline (0.552).
\textit{Bottom:} Random forest feature importance and key statistical results.}
\label{fig:nbody_stats}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{v4_full_results.png}
\caption{v4 framework full experimental results.
\textit{Top left:} All 256 ECA rules ranked by composite $C$; C4 rules (green) clearly dominate.
\textit{Top right:} v3$\to$v4 improvement in Cohen's $d$ (17.6$\to$38.4 for 1D ECA).
\textit{Centre:} Opacity vector space for 1D ECA and 2D Life-like rules; C4 cluster annotated.
\textit{Bottom:} C4/C3 separation ratio across substrates and full statistical summary.}
\label{fig:v4_results}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{eeg_synthetic_analysis.png}
\caption{Synthetic EEG results across five brain states.
\textit{Top row:} Representative EEG traces for each state.
\textit{Centre row:} Component metric weights ($w_H$, $w_{\text{OP},s}$,
$w_{\text{OP},t}$, $w_T$, $w_G$) per state.
\textit{Bottom:} Composite $C$ rankings; seizure correctly identified
as lowest (frozen), active task second-lowest (chaotic).
The parameter-free tanh spatial opacity formulation transfers to this
substrate without modification.}
\label{fig:eeg_synthetic}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{eeg_v7_analysis.png}
\caption{EEG sleep stage complexity analysis (v7 framework).
8-channel synthetic EEG, 250\,ms bins, 30 epochs per stage.
\textit{Top:} Composite $C$ by sleep stage; N3 deep sleep ranks first
due to strong temporal opacity ($w_{\text{OP},t}\!=\!0.911$) from slow
delta wave synchrony.
\textit{Bottom left:} Metric weight heatmap by stage.
\textit{Bottom centre:} Results and interpretation.
Ranking: N3\,$>$\,REM\,$>$\,N1\,$>$\,Wake\,$>$\,N2.}
\label{fig:eeg_sleep}
\end{figure}
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Direct validation of P6 (co-evolutionary dynamics).}
    The spatial Prisoner's Dilemma experiment demonstrates that $C$
    peaks precisely when and because mutual evolutionary pressure
    between co-adapting populations is active.
    Three control games confirm specificity: coordination, anti-coordination,
    and ordered cooperation all return $C\!\approx\!0$.

  \item \textbf{Cross-domain convergence.}
    The PD peak at $T\!=\!1.80$ matches the Nowak-May critical regime
    identified by evolutionary game theory.
    The N-body peak matches Adams et al.'s island of stability identified
    by physical fine-tuning arguments.
    In both cases, independent domain science and information-theoretic
    measurement converge on the same critical regime.
\end{enumerate}

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{cross_experiment_analysis.png}
\caption{Cross-substrate statistical analysis.
\textit{Top row:} Score distributions by class for 1D ECA (256 rules),
2D Life-like (21 rules), and N-body (100 parameter points).
\textit{Centre row:} Separation ratio, Cohen's $d$, and F1 score across
substrates.
\textit{Bottom:} Full 1D ECA score ranking and summary statistical table.}
...
computationally universal behaviour across five distinct substrates.
...
The spatial Prisoner's Dilemma adds two new contributions.
First, it directly validates P6 (co-evolutionary dynamics): $C$ peaks
when and only when mutual evolutionary pressure between populations is
active, and three controls confirm this specificity.
Second, the framework independently recovers the Nowak-May critical
temptation $T\!\approx\!1.8$, providing cross-domain convergence with
evolutionary game theory parallel to the N-body convergence with
physical fine-tuning arguments.
Together these results support the central claim: the composite detects
a substrate-agnostic property that independent scientific traditions
have each been measuring from the outside.
...
Spatial PD       & 21   & ---   & ---  & ---
  & Peak $C\!=\!1.06$ at $T\!=\!1.80$; matches Nowak-May \\
PD controls      & 3    & ---   & ---  & ---
  & Stag Hunt, Minority: $C\!\approx\!0$ confirmed \\
\bottomrule
\end{tabular}
\end{table}
